...
def parse(file_path):
    ...
    for filename in file_path:
        ...
        item.setdefault('VERYCD_ID', os.path.basename(filename))
        ...
        item.setdefault('ACTOR', str([i.text() for i in Q('#iptcomActor .iptcom-info a').items()]))

        item.setdefault('KIND', Q('#iptcomKind').text().replace('古典类型', '').replace(':', '').strip().split(' ')[0])
        ...
        if Q('#iptcomVersion'):
            item.setdefault('VERSION', Q('#iptcomVersion').text().replace(u'版本', u'').
                            replace(u':', u'').strip().split(' ')[0])
        else:
        ...
        item.setdefault('ITEM_PUBLISH_TIME',
                        int(
                            time.mktime(
                                datetime.datetime.strptime(
                                    Q('.block11 li span.date-time').eq(0).text(), '%Y/%m/%d %H:%M:%S').timetuple()
                            )
                        ))

        item.setdefault('ITEM_UPDATE_TIME',
                        int(
                            time.mktime(
                                datetime.datetime.strptime(
                                    Q('.block11 li span.date-time').eq(1).text(), '%Y/%m/%d %H:%M:%S').timetuple()
                            )
                        ))
